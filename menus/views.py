...
def index(request):
    ...
    return render_to_response('menus/index.html', {'year': datetime.today().year,'month':datetime.today().month, 'day':datetime.today().day})
...
def semaine(request,annee,mois,jour):
   ...
   # journees = Journee.objects.all()#Poll.objects.all().order_by('-pub_date')[:5]
    ...
    return render_to_response('menus/semaine.html', {'year': annee,'month':mois, 'day':jour,'journees':journees,'next':next,'prev':prev})
...
def semainecomplete(request,annee,mois,jour):
   ...
   # journees = Journee.objects.all()#Poll.objects.all().order_by('-pub_date')[:5]
    journees = JourneePensionComplete.objects.filter(jour__lte=dateDimanche).filter(jour__gte=dateLundi).order_by('jour')
    return render_to_response('menus/semaineComplete.html', {'year': annee,'month':mois, 'day':jour,'journees':journees,'next':next,'prev':prev})
