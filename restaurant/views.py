...
def plats(request):
	...
	return render_to_response('restaurant/plats.html', {'year': datetime.today().year,'month':datetime.today().month, 'day':datetime.today().day,"tout":tout})

@login_required
def index(request):
	fs=Fournisseur.objects.all()
	return render_to_response('restaurant/index.html', {'year': datetime.today().year,'month':datetime.today().month, 'day':datetime.today().day,'fournisseurs':fs})
...
def fournisseurReset(request,fournisseur):
	...
	return render_to_response('restaurant/fournisseur.html',{"tuples":[getTupleFournisseur(fournisseur)],"print":False})
@login_required
...
def jour(request,annee,mois,jour):
	...
	return render_to_response('restaurant/jour.html', {'year': datetime.today().year,'month':datetime.today().month, 'day':date.today(),"resas":resas,'dateDemandee':jou,'prochainsJours':prochainsJours,'total':t,"nbResa":nbResa,"parNb":parNbClasse,"pensClass":pensClass,"passClass":passClass})
