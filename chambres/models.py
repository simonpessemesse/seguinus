...
nomMois=["zero","janvier","fevrier","mars","avril","mai","juin","juillet","aout","septembre","octobre","novembre","decembre"]

def joliePeriode(dateArrivee,dateDepart,avecJours=False):
		if dateArrivee.month==dateDepart.month:
			if avecJours:
				return jours[dateArrivee.weekday()]+" "+str(dateArrivee.day)+" au "+jours[dateDepart.weekday()]+" "+str(dateDepart.day)+" "+nomMois[dateDepart.month]
			else:
				return str(dateArrivee.day)+" au "+str(dateDepart.day)+" "+nomMois[dateDepart.month]
		else:
			if avecJours:
				return  jours[dateArrivee.weekday()]+" "+str(dateArrivee.day)+" "+nomMois[dateArrivee.month]+" au "+jours[dateDepart.weekday()]+" "+str(dateDepart.day)+" "+nomMois[dateDepart.month]
			else:
				return str(dateArrivee.day)+" "+nomMois[dateArrivee.month]+" au "+str(dateDepart.day)+" "+nomMois[dateDepart.month]
...
class Chambre(models.Model):
	...
	def __str__(self):
		...
		if self.avecWC is True:
			wc="WC"
		return self.nom+ "  "+str(self.grandsLits)+" gd + "+str(self.petitsLits)+" petits  "+wc+" "+ str(self.note) 
	def capacite(self):
...
class TourOperateur(models.Model):
	...
	def nettoieAllotements(self):
		auj=date.today()
		aNettoyer=Reservation.objects.filter(client__tourOperateur=self).filter(client__nom="allotement "+self.nom).filter(dateArrivee__lt=auj+timedelta(self.nbJoursAnnulationAllotement))
