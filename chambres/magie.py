...
def caseTab(checked, chambre, jour):
    ...
    pre = "<td title=\"" + chambre.nom + " le " + formatte(
        jour) + "\"><input type=\"checkbox\" name=\"" + "magie" + "\" value=\"" + value + "\""
    post = ">" + "</td>"
    if (checked):
        result = " checked "
    else:
...
def interieur(resas):
    ...
    for jour in liste:
        ...
        result += "<td><INPUT type=\"text\" size=\"1\" value=\"" + str(totalPlDortoir) + "\" name=\"" + str(
            jour[0]) + "\"></td>"
...
def index(request, resa):
    ...
    if request.method == 'POST':  # If the form has been submitted...
        dortoirs = [ddd for ddd in request.POST.lists() if
                    ddd[0] != "magie" and ddd[0] != "referer" and ddd[0] != "csrfmiddlewaretoken"]
        resumeDortoir = dict()
        for d in dortoirs:
        ...
        for ((c1, c2), valeur) in list(dicoParSejour.items()):
            ...
            resa = Reservation(client=laResa.client, dateArrivee=c1, dateDepart=c2, chambres=len(valeur),
                               placesDortoir=plDor)
            resa.save()
            for ch in valeur:
    ...
    if "HTTP_REFERER" in request.META:
        ...
        referer = ""
    return render_to_response('chambres/magie.html', {'tableau': tableau, 'id': resa, 'referer': referer},
