	...
	def __init__(self,resas,booking=False,nodp=False):
		...
		for resa in resas:
			...
			if booking:
				...
				if nom.find("nodp")<0 and nom.find("no dp")<0 and "allotement" not in nom: # si il n'y a pas no dp (pas de demi pension) dans le nom
					onRajoute=True
				else:
					onRajoute=False
			if nodp:
			...
			if onRajoute:
				...
				if nom in dicParNom:
					...
					dicParNom[nom]=(resa.nbPersonnes,list(resa.chambresAssignees),resa.dateArrivee,resa.dateDepart,resa.nbEnfants)
		dicParNb={}
		total=0
		totalKid=0
...
def convertisResaEnReservationDeResto(resa):
	r=ReservationDeResto(resa.client.nom,resa.nbPersonnes(),resa.chambresAssignees.all(),resa.dateDepart,resa.dateArrivee,resa.nbEnfants)
	return r
...
def faisStatsRepasSoir(dateDemandee, nouveaux, anciensQuiChangent, menages, resas ):
	...
	for r in resaRestoJour:
		if r.nbAnciens>0:
			anciensByPass.append(ReservationDeResto(r.nomJoli(),r.nbAnciens,[],dateArrivee=dateDemandee,dateDepart=dateDemandee,nbEnfants=r.nbEnfants))
		if r.nbNouveaux>0:
			nouveauxByPass.append(ReservationDeResto(r.nomJoli(),r.nbNouveaux,[],dateArrivee=dateDemandee,dateDepart=dateDemandee,nbEnfants=r.nbEnfants))
		...
		if r.client.tourOperateur:
			if r.client.tourOperateur.nom=="Chemins des suds" and r.nbPersonnes()>=5:
				if dateDemandee.weekday()==3:
					lundi=Reservation.objects.filter(dateArrivee=dateDemandee-timedelta(4)).filter(dateDepart=dateDemandee-timedelta(3))
					for rr in lundi:
