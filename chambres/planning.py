...
def faisPlanning(dateDemandee):
	reservations=Reservation.objects.filter(dateArrivee__lte=dateDemandee).filter(dateDepart__gt=dateDemandee).prefetch_related("chambresAssignees").select_related("client")
	dico=dict()
	assign=[]
...
def lienResa(resa,dat=None,ch=None,retourLigne=True):
	...
	if len(resa.client.nom)>6:
		nom=nom[0:6]
	lr="<small>"+ita+"<a "+ tooltip+ " href=\"/admin/chambres/client/"+str(resa.client.id)+"\" "+prot+">"+nom+"</a>"+rl+li+"</small>"
...
def planningToday(request):
	...
	return HttpResponseRedirect("/chambres/planning/"+str(datetime.today().year)+"/"+str(datetime.today().month)+"/"+str(datetime.today().day)+"/"+str(suiv.year)+"/"+str(suiv.month)+"/"+str(suiv.day))
...
def calculeCouleursClients(dateDeb,dateFin):
	...
	for i in range(0,(dateFin-dateDeb).days+1):
		...
		reservations=Reservation.objects.filter(dateArrivee__lte=d).filter(dateDepart__gt=d).prefetch_related("chambresAssignees").select_related("client")
		for r in reservations:
...
def planning(request,annee,mois,jour,anneeF,moisF,jourF):
	...
	for i in range(0,(dateFin-dateDeb).days+1):
		...
		html+=cellv("<a href=\"/chambres/"+str(dateDeb+timedelta(i)).replace("-","/")+"\">"+formateDate(dateDeb+timedelta(i))+"</a>")
		for ch in chambres:
