...
from chambres.models import Chambre,Souci,Client,Reservation,Tache,Amour,CacheJour,Entite,TourOperateur,ClientForm
...
class OneDayStats:
	def __init__(self,dateDemandee):
		...
		self.reservations=Reservation.objects.filter(dateArrivee__lte=dateDemandee).filter(dateDepart__gt=dateDemandee).select_related("client").prefetch_related("chambresAssignees").prefetch_related("client__arrhe_set").select_related("client__tourOperateur")
		totalChambres=set([ch for ch in Chambre.objects.all()])
		self.menages=self.reservations.filter(dateDepart__gt=dateDemandee).filter(dateArrivee__lt=dateDemandee).select_related("client").prefetch_related("chambresAssignees").prefetch_related("client__arrhe_set").select_related("client__tourOperateur")


		self.blancs=Reservation.objects.filter(dateDepart=dateDemandee).select_related("client").prefetch_related("chambresAssignees").prefetch_related("client__arrhe_set").select_related("client__tourOperateur")


		arriv=Reservation.objects.filter(dateArrivee=dateDemandee).select_related("client").prefetch_related("chambresAssignees").prefetch_related("client__arrhe_set").select_related("client__tourOperateur")
		...
		self.repas=repas.faisStatsRepasSoir(self.dateDemandee, self.nouveaux, anciensQuiChangent, self.menages, list(self.reservations) )
...
def vuePapier(request,annee,mois,jour,anneeF,moisF,jourF,etatResas=False):
	...
	if request.method == 'POST': # If the form has been submitted...
		...
		if form.is_valid(): # All validation rules pass
			...
			return HttpResponseRedirect('/chambres/vuePapier/%s/%s/%s/%s/%s/%s#%s' % (annee,mois,jour,anneeF,moisF,jourF,j.isoformat()))
	...
	resas=Reservation.objects.filter(dateArrivee__gte=dateDeb).filter(dateDepart__lte=dateFin).select_related("client","client__tourOperateur").prefetch_related("client__arrhe_set","client__arrhe_set__paiement_set")
	resasLimitesAvant=Reservation.objects.filter(dateArrivee__lt=dateDeb).filter(dateDepart__gt=dateDeb).filter(dateDepart__lte=dateFin).select_related("client","client__tourOperateur").prefetch_related("client__arrhe_set","client__arrhe_set__paiement_set")

	resasLimitesApres=Reservation.objects.filter(dateArrivee__lte=dateFin).filter(dateDepart__gt=dateFin).filter(dateArrivee__gte=dateDeb).select_related("client","client__tourOperateur").prefetch_related("client__arrhe_set","client__arrhe_set__paiement_set")

	resasLimitesAvantApres=Reservation.objects.filter(dateArrivee__lt=dateDeb).filter(dateDepart__gt=dateFin).select_related("client","client__tourOperateur").prefetch_related("client__arrhe_set","client__arrhe_set__paiement_set")
...
#		print "avap",r.client.nom,r
	...
	for j in jours:
		...
		jour.totSures=sum(r.nbCh() for r in resas if ((not r.client.optionJusquau) and (not "allotement" in r.client.nom)))	
...
#		d=dateDeb+timedelta(i)
	...
	return render_to_response('chambres/vuePapier.html', {'jours':tout,'largeur':(dateFin-dateDeb).days*210},context_instance=RequestContext(request))
...
def globalStat(request,annee,mois,jour,anneeF,moisF,jourF,imprime=False,futur=False):
	...
	for i in range(0,(dateFin-dateDeb).days+1):
		...
		res.men=(max(res.nbMenages,res.nbMenageTheorique)*10/2+max(res.nbBlancs,res.nbBlancsTheoriques)*10)/10+(res.nbBlancsDortoir*100/20*4+res.nbMenageDortoir*100/20*2)/100
...
#	chambres.sort(lambda (x,xx),(y,yy): yy-xx)
	...
	for i in range(len(result)):
		...
		if(i%modulo==0):
			xticks.append("{label:\""+str(result[i].dateDemandee.day)+"/"+str(result[i].dateDemandee.month)+"\",v:"+str(i)+"}" )
		else:
	...
	graph1=",".join(graph1)
	graph2=",".join(graph2)
	return render_to_response('chambres/global.html', {'nbNuitees':nbNuitees,'nbRepas':nbRepas,'year': annee,'month':mois, 'day':jour, 'result':result,'graph1':graph1,'graph2':graph2  ,'xticks':xticks,'chTicks':chTicks,'chGraph':chGraph,'lenCh':chambres[0][1]+1,'minCh':chambres[-1][1]-1,'joursStat':joursStat,'imprime':imprime})


@login_required
def globalStatToday(request):
	suiv=datetime.today()+timedelta(30)
	return HttpResponseRedirect(reverse("globalStat",args=[datetime.today().year,datetime.today().month,datetime.today().day,suiv.year,suiv.month,suiv.day]))

@login_required
def globalStatTodayPrint(request):
	suiv=datetime.today()+timedelta(30)
	return HttpResponseRedirect(reverse("globalStatPrint",args=[datetime.today().year,datetime.today().month,datetime.today().day,suiv.year,suiv.month,suiv.day]))
...
def faisStat(annee,mois,jour):
	...
	o.resasOptionnellesDepassees=list(Client.objects.filter(optionJusquau__lte=dateDemandee).filter(tourOperateur__isnull=True).exclude(nom__icontains="booking").prefetch_related("reservation_set"))
	...
	dico= {'year':	annee,'month':mois,
			'day':jour,'reservations':o.resas,'menages':o.menages,'blancs':o.blancs,'arrivees':o.arrivees,'next':next,'prev':prev,'error':o.error,'libre':o.libre,'nbCh':o.nbCh,'nbSingle':o.nbSingle,'nbDouble':o.nbDouble,'nbTwin':o.nbTwin,'nbTriples':o.nbTriples,'nbQuadruples':o.nbQuadruples,'nbQuintuples':o.nbQuintuples,'nbAll':o.nbAll,'nbPlacesLibreDortoir':o.nbPlacesLibreDortoir,'nbDortoir':o.nbDortoir,'nomJour':o.nomJour,'taches':tToday,'repas':o.repas,'changements':o.changements,'departs':o.departs, 'prochainsJours':prochainsJours,'dateDemandee':dateDemandee,'oneDayStat':o }
	return dico
...
def editClient(request,client):
	if request.method == 'POST': # If the form has been submitted...
		...
		ResaFormSet = modelformset_factory(Reservation,fields=('dateArrivee', 'dateDepart','chambres', 'chambresSingle','chambresDoubles','chambresTwin','placesDortoir','chambresTriples','chambresQuadruples','chambresQuintuples','chambresAssignees'),can_delete=True)
