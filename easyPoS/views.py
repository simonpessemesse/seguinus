...
def arrhes(request):
	if("kdo" in request.GET):
		arrhes=Arrhe.objects.all().order_by('-id').filter(client__reservation__isnull=True)#.filter(paiement__facture__isnull=True)
		res=set()
		for a in arrhes:
...
def ajoutkdo(request):
	...
	return render_to_response("easyPoS/addBonKdo.html",{'nonEncaissesForm':NonEncaissesForm,'paiementForm':PaiementForm,'bonKdoForm':BonKdoForm},context_instance=RequestContext(request))
...
def editArrhes(request,clientId):
	...
	return render_to_response('easyPoS/editArrhes.html', {'nonEncaissesForm':NonEncaissesForm,'paiementForm':PaiementForm,'client':client,'arrhes':arrhes,'paiements':paiements,'modifs':modifs},context_instance=RequestContext(request))
...
def detail(request, facture_id,imprime=False,devis=False):
	...
	return render_to_response('easyPoS/detail.html', {'f': p,'donnees':donnees,'imprime':imprime,'today':date.today(),'devis':devis})
...
def stats(request,annee=None,mois=None,jour=None,anneeF=None,moisF=None,jourF=None):
	...
	if annee and not anneeF:
		...
		factures=Facture.objects.filter(dateVente__gte=date.today()).filter(dateVente__lte=date.today()+timedelta(1)).filter(etat='V')
	prods=Produit.objects.all().order_by('nom')
	...
	for f in fact:
		...
		for p in paie:
			if p.date < f.dateVente-timedelta(0.3) :
				deb+="pb av fact "+str(f.numero)+" du "+str(f.dateVente)+" qui a un paiement du "+str(p)+"\n"
	return deb
...
def recapJour(request,annee,mois,jour,cherche=None):
	...
	if cherche:
		...
		factures=Facture.objects.filter(entreprise=ent).filter(etat="V").filter(dateVente__year=int(annee)).filter(dateVente__month=int(mois)).filter(dateVente__day=int(jour))
	lignes={}
	for f in factures:
...
def remisesCheque(request,annee,mois,jour):
	...
	for i in range(len(rems)):
		...
		r.rPrec=rems[i].date
		touts=Paiement.objects.filter(date__gt=r.rPrec).filter(date__lt=r.rCour).filter(moyenPaiement=3) #pour cheque
		r.chs=[]
		for t in touts:
...
class ResumePersonnalise:
	...
	def remisesCheque(self):
		...
		for k,v in chequeParMontant.items():
			chtt.append((k,v))
		self.extras={"afficheCheques":True,"nb":self.nb,"total":self.total,"cheques":self.chs,"chequeParMontant":chtt}
	...
	def rapportEtFactures(self):
		...
		facturesDuJour=Facture.objects.filter(entreprise=ent).filter(dateVente__gte=self.dateD).filter(dateVente__lte=self.dateF).order_by('numero').filter(etat='V')
	...
	def resultat(self,request):
		...
		arguments= {"prochain":self.prochain,"prev":self.prev,"date":self.dateD,"rapport":r,"factures":facturesDuJour,"afficheEncaissementsDetailles":self.afficheEncaissementsDetailles,"titre":self.titre,"user":user,"debiteurs":debiteurs}
...
def encaissementsInsecure(annee,mois):
	...
	for j in joursAEtudier:
		...
		ent=DonneesEntreprise.objects.get(id=preferences.ENTREPRISE)
		paiements=Paiement.objects.filter(entreprise=ent).filter(date__month=mois).filter(date__year=annee).filter(date__day=jour)
		facturesDuJour=Facture.objects.filter(entreprise=ent).filter(dateVente__month=mois).filter(dateVente__year=annee).filter(dateVente__day=jour).order_by('numero').filter(etat='V')
...
#	encaissements.reverse()
	return render_to_response('easyPoS/encaissementParPeriodes.html', {"date":dateD,"prochain":prochain,"prev":prev,"encaissements":encaissements,"total":total,"totalGeneral":totalGeneral})
...
def doublesQuotidien(request,annee,mois,jour,imprime=False):
	...
	ent=DonneesEntreprise.objects.get(id=preferences.ENTREPRISE)
	factures=Facture.objects.filter(entreprise=ent).filter(dateVente__month=mois).filter(dateVente__year=annee).filter(dateVente__day=jour).order_by('numero').filter(etat='V')
	col1,col2,col3=getCols(factures)

	return render_to_response('easyPoS/doublesQuotidien.html', 
			{"date":dateD,"prochain":prochain,"prev":prev,"factures":factures,"col1":col1,"col2":col2,"col3":col3,"all":factures,"imprime":imprime})
...
def doublesMensuels(request,annee,mois):
	...
	ent=DonneesEntreprise.objects.get(id=preferences.ENTREPRISE)
	factures=Facture.objects.filter(entreprise=ent).filter(dateVente__month=mois).filter(dateVente__year=annee).order_by('numero').filter(etat='V')
	col1,col2,col3=getCols(factures)

	return render_to_response('easyPoS/doublesQuotidien.html', 
			{"date":dateD,"prochain":prochain,"prev":prev,"month":True,"factures":factures,"col1":col1,"col2":col2,"col3":col3,"all":factures})
