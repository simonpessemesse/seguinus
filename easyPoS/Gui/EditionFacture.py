...
from easyPoS.models import Facture,Categorie,Produit,LigneFacture,Paiement,MoyenPaiement,DonneesEntreprise,Arrhe,LogFacture,RelationReservationFacture
...
#			self.lblReglementRestantDu.setText(" ")
	...
	def prodClicked(self,prod):
		...
		ligne=LigneFacture(produit=pr,famille=pr.famille.nom,facture=self.fact,prixUnitaireFinal=pr.prix,nomProduitFinal=pr.nom,tauxTvaFinal=pr.tva.taux,position=time.time())
		ligne.save()
...
#		self.lignes.removeRow(n)
		if li.produit:
			log=Log(facture=self.fact,description="Suppression de "+str(li.quantite)+" "+li.produit.nom+" a "+ str(li.prixUnitaireFinal)+" euros num "+str(li.id))
			log.save()
			self.addLog(log)
		li.delete()
		self.initLignes()
#		self.pasDeFaussesInfo()
	...
	def chQuantite(self,newVal):
		li=self.sender().ligne
		tot=self.sender().tot
		log=Log(facture=self.fact,description="Changement Quantite de "+str(li.quantite)+" a "+str(newVal)+" " +li.produit.nom+ " num "+str(li.id))
...
#		self.pasDeFaussesInfo()
	...
	def chPrix(self,newVal):
		...
		if li.produit and (li.produit.id==173 or li.produit.id==175):
			return
		log=Log(facture=self.fact,description="Changement prix de "+str(li.prixUnitaireFinal)+" a "+str(newVal) +" "+li.produit.nom+ " num "+str(li.id))
...
# TODO quand on supprime le moyen de paiement ne pas supprimer l'arrhe eventueellement liee
		...
		if parDefaut==100:
			try:
				...
				if dudu<0:
					...
					parDefaut=dudu/2
		d,ok=QtGui.QInputDialog.getDouble(self, self.tr("Reglement"),self.tr("Combien:"), parDefaut, -1000000, 1000000, 2)
		if ok:
	...
	def addCB(self,montant):
		...
		p=Paiement(entreprise=ent,montant=Decimal(montant),moyenPaiement=moyCB,facture=self.fact,date=datetime.now())
		p.save()
		self.initListeReglements()
	...
	def addCheque(self,montant):
		...
		ent=self.fact.entreprise
		p=Paiement(entreprise=ent,montant=Decimal(montant),moyenPaiement=moyCheque,facture=self.fact,date=datetime.now())
		p.save()
		self.initListeReglements()
	...
	def addEspece(self,montant):
		...
		ent=self.fact.entreprise
		p=Paiement(entreprise=ent,montant=Decimal(montant),moyenPaiement=moyEspece,facture=self.fact,date=datetime.now())
		p.save()
		self.initListeReglements()
	...
	def Dvirement(self):
		...
		if ok:
			ent=self.fact.entreprise
			p=Paiement(entreprise=ent,montant=Decimal((str(d))),moyenPaiement=moyANCV,facture=self.fact,date=datetime.now())
			p.save()
			self.initListeReglements()
	...
	def addANCV(self,montant):
		...
		ent=self.fact.entreprise
		p=Paiement(entreprise=ent,montant=Decimal(montant),moyenPaiement=moyANCV,facture=self.fact,date=datetime.now())
		p.save()
		self.initListeReglements()
	...
	def rendMonnaie(self):
		...
		if(totalDu<0):
			...
			ent=self.fact.entreprise
			p=Paiement(entreprise=ent,montant=Decimal(totalDu),moyenPaiement=rendu,facture=self.fact,date=datetime.now())
			p.save()
			self.initListeReglements()
	...
	#	self.listeProduits.setSpacing(5)
		...
		pro=Categorie.objects.get(id=1).produit_set.filter(actif=True).order_by('nom') #choppe les produits frequemment vendu
		for j in pro:
	...
	def valideMoiCa(self):
		...
		if estHotel:
			if nbPers!=nbTs:
				ret=QtGui.QMessageBox.warning(self,"..","Attention, il y a peut être "+str(nbPers)+" qui dorment dans cette facture et vous avez facturé "+str(nbTs)+" taxe(s) de séjour",                   QtGui.QMessageBox.Ok     | QtGui.QMessageBox.Cancel,
...
#		self.close()
	...
	def supprime(self):
		...
		for p in paiements:
			if p.arrhe:
				QtGui.QMessageBox.warning(self,"Attention","Il y a des arrhes associees, veuillez les supprimer avant de supprimer la facture", QtGui.QMessageBox.Ok, QtGui.QMessageBox.Ok);
				return
		ret = QtGui.QMessageBox.warning(self, "Attention",
...
#		self.montantMontre=True
	...
	def ecrireProdQuantite(self,prodQuantite):
		for key,item in list(prodQuantite.items()):
			(prod,prix)=key
			ligne=LigneFacture(produit=prod,famille=prod.famille.nom,facture=self.fact,prixUnitaireFinal=prix,nomProduitFinal=prod.nom,tauxTvaFinal=prod.tva.taux,quantite=item,position=time.time())
			ligne.save()
			ligne.position=ligne.id
			ligne.save()
	...
	def factoriseLignes(self):
		...
		for l in lignes:
			if l.libelle:
				...
				ligne=LigneFacture(produit=l.produit,facture=self.fact,famille=l.famille,prixUnitaireFinal=l.prixUnitaireFinal,nomProduitFinal=l.nomProduitFinal,tauxTvaFinal=l.tauxTvaFinal,quantite=l.quantite, position=time.time())
				ligne.save()
				ligne.position=ligne.id
				ligne.save()
