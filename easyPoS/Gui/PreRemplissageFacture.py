...
def ajouterNProd(n,prod,fact,prix=None):
	if n>0:
		if prix:
			...
			pr=prod.prix
		ligne=LigneFacture(produit=prod,facture=fact,famille=prod.famille.nom,prixUnitaireFinal=pr,nomProduitFinal=prod.nom,tauxTvaFinal=prod.tva.taux,quantite=n,position=time.time())
		ligne.save()
