...
class ListePrincipaleModel(QtCore.QAbstractListModel):
    def __init__(self, parent=None, brouillons=True, fen=None, val50=False, valnp=True, viei=False, devis=False, *args):
        ...
        if (valnp):
            ...
            factNonPaye2 = set(
                Facture.objects.filter(entreprise=ent).filter(cacheTotalDu__lt=0).filter(etat='V')).union(
                set(factNonPaye))
...
class Fenetre(QtGui.QMainWindow):
    ...
    def cloneMoiCa(self, fact):
        ...
        for i in fact.lignefacture_set.all():
            ligne = LigneFacture(produit=i.produit, famille=i.famille, facture=facture,
                                 prixUnitaireFinal=i.prixUnitaireFinal, quantite=i.quantite, position=i.position,
                                 tauxTvaFinal=i.tauxTvaFinal)
            ligne.save()
        facture.save()
    ...
    def chkChanged(self):
        ...
        self.listePrincipaleModel = ListePrincipaleModel(self, brouillons=bro, fen=self, val50=val50, valnp=valnp,
                                                         viei=viei, devis=devis)
