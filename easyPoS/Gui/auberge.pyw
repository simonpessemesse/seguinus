...
class MinuteurLanceurDeTaches(QThread):
    ...
    def envoieCaisses(self):
        ...
        resume = str(
            resumeMensuelInsecure(None, le15DuMoisDernier.year, le15DuMoisDernier.month).content.decode("UTF-8"))
        encaissements = str(
            encaissementsInsecure(le15DuMoisDernier.year, le15DuMoisDernier.month).content.decode("UTF-8"))
    ...
    def demarreServeur(self):
        ...
        if preferences.LANCER_SERVEUR:
            try:
                ...
                self.enfant = subprocess.Popen(["python", "manage.py", "runserver"], cwd=os.path.abspath(".."),
