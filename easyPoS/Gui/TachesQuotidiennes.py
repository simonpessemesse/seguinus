...
def mutuelleInscriptionDesinscription():
	...
	if t.day in [1,2,3]:
		...
		for c in contrats:
			if c.dateDebut:
				d=t-timedelta(15)
				if d.year==c.dateDebut.year and d.month==c.dateDebut.month and ((not c.dateFin) or ((c.dateFin-c.dateDebut).days)>32):
...
def envoieOccupationOfficeTourisme():
	...
	for i in range(7):
		...
		txt+="il y a {0} places libres au dortoir et {1} chambres\n".format(stats.nbPlacesLibreDortoir,30-stats.chambresReservees)
...
def creerTachePicNicPlusieursJours(r,tourOp=False):
	for i in range((r.dateDepart-r.dateArrivee).days):
		...
		if tourOp:
			desc=str(r.nbPersonnes())+" pique nique pour "+r.client.nom+" de "+r.client.tourOperateur.nom+" le "+str(jour)+" ?? Se renseigner "
		else:
			desc=str(r.nbPersonnes())+" pique nique pour "+r.client.nom+" le "+str(jour)+" ?? Se renseigner "
...
def creerTachesPiqueNique():
	...
	for r in resas:
		if r.client.tourOperateur:
			...
			if (r.client.tourOperateur.nom=="Inn Travel" and "Allotement" not in r.client.nom )	or 	 ("Chemins" in r.client.tourOperateur.nom and r.nbPersonnes()>4):
				desc=str(r.nbPersonnes())+" pique nique pour "+r.client.nom+" de "+r.client.tourOperateur.nom+" le "+str(t)
...
def remplisRegistreDuPersonnel():
	...
	for r in reg:
		creeSiExistePasTache("ajouter dans registre personnel "+str(r.individu)+" debut le "+str(r.debut),heure=time(1,3,2))
