...
class Facture(models.Model):
    ...
    def estFactureHotel(self):
        nombrePersonneParLigne = [(1, [2, 9, 10, 13, 107, 108, 118, 119, 136, 142, 143, 148, 149, 169]),
                                  (2, [7, 8, 120, 121, 134, 135, 150]), (3, [11, 137, 157]), (4, [12, 138, 139, 151]),
                                  (5, [163])]
        idDesLignesDhotel = []
        gensParId = {}
    ...
    def creeAvoir(self, aZero=False):
        ...
        for i in self.lignefacture_set.all().order_by("position"):
            if not i.libelle:
                ligne = LigneFacture(produit=i.produit, facture=f, famille=i.famille,
                                     prixUnitaireFinal=i.prixUnitaireFinal, nomProduitFinal=i.nomProduitFinal,
                                     tauxTvaFinal=i.tauxTvaFinal, position=i.position, quantite=-i.quantite)
                ligne.save()
            else:
        ...
        if aZero:
            for i in self.lignefacture_set.all().order_by("position"):
                if not i.libelle:
                    ligne = LigneFacture(produit=i.produit, facture=f, famille=i.produit.famille.nom,
                                         prixUnitaireFinal=i.prixUnitaireFinal, nomProduitFinal=i.nomProduitFinal,
                                         tauxTvaFinal=i.produit.tva.taux, position=i.position + 100,
                                         quantite=i.quantite)
                    ligne.save()
        f.save()
...
class Paiement(models.Model):
    ...
    def __str__(self):
        return self.entreprise.denomination + str(self.montant) + " Euros en " + str(self.moyenPaiement) + " le " + str(
