...
def envoieCaisses(resume, encaissements, desc):
    ...
    sendMail(
        ...
        "a envoyer a la comptable",
        "NE PAS OUBLIER DE RAJOUTER LES DEUX DERNIERS RELEVES DE BANQUE \n Voici le resume et les encaissements " + desc,
        [nCaisse, nResum]
    )
    os.remove(nCaisse)
    os.remove(nResum)
...
def dumpDatabase():
    ...
    enfant = subprocess.Popen(
        ["python3", "manage.py", "dumpdata", "--indent=1", "chambres", "restaurant", "collectage", "easyPoS", "menus",
         "taches", "telephones"], cwd=os.path.abspath("."), stdout=fichier, stderr=subprocess.STDOUT)
    enfant.wait()
    fichier.close()
...
def envoieASelvaListe():
    ...
    for (j, f) in joursFichiers:
        ...
        urll = "http://127.0.0.1:8000/chambres/" + str(dem.year) + "/" + str(dem.month) + "/" + str(
            dem.day) + "/" + "imprimerCh"
        logging.info(urll)
...
paramAndFunction = {'commitD': autocommitLesDonnees, 'pushD': autoPushLesDonnees, 'mailD': envoieParMailLesDonnees,
                    'getLastVer': getLastCodeVersion, 'getDBVer': getLastDatabaseVersion, 'dump': dumpDatabase,
                    'selva': envoieASelvaListe}
