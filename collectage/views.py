...
def getDicoResumes(dateDeb,dateFin,contrat=None,imprimer=False):
	...
	for j in jours:
		...
		if contrat:
			...
			r.heuresPlanifiees=HeurePlanifiee.objects.filter(jour=j).filter(employe=contrat.individu).order_by("heureDebut")
	...
	if contrat:
		...
		resumTotal.enCours=""
		if imprimer:
			return {"resumes":resumes,"emp":contrat,"dd":dateDeb,"df":dateFin-timedelta(1),"semaines":semaines, "today":date.today()}
		...
		if c.dateDebut and j>=c.dateDebut and ((c.dateFin and j<=c.dateFin) or not c.dateFin):
			if not j.isoweekday() in joursDeConge:
				if c.demiJourneeConge and j.isoweekday()==c.demiJourneeConge:
					nvelle=HeurePlanifiee(jour=j,employe=c.individu,tempsTravaille=moyenneParJour/2,nbRepasPris=1)
					nvelle.save()
				else:
...
def formateDeuxDatesPourAfficher(deb,fin):
	return str(deb.year)+"/"+ str(deb.month)+"/"+ str(deb.day)+"/"+ str(fin.year)+"/"+ str(fin.month)+"/"+ str(fin.day)+"/"
...
def moisMail(request,annee,mois,preremplis=False,genereTaches=False):
	...
	jm=dd-timedelta(1)
	jmPlus=date((date(int(annee),int(mois),15)+timedelta(30)).year,(date(int(annee),int(mois),15)+timedelta(30)).month,1)
	...
	for c in commeDhab:
		if preremplis:
			faisPreRemplissage(request,dd.year,dd.month,dd.day,premierJourMoisSuivant.year,premierJourMoisSuivant.month,premierJourMoisSuivant.day,c.id)
		...
		if c.dateDebut and c.dateFin and (c.dateDebut>dd or c.dateFin<premierJourMoisSuivant or c.nombreHeuresSemaine<35):
	...
	return render_to_response('collectage/salaireMois.html',{"premier":dd,"hab":commeDhab,"nvx":nouvellesEmbauches,"extras":extras,"finsContrats":finContrats,"modifs":modifs,"moisP":moisP,"moisS":moisS,"jourP":jourP,"jourS":jourS})
...
def contrat(request,numContrat):
	...
	return render_to_response('collectage/'+cc.get_typeDuContrat_display()+'.html',{'object':cc,'entreprise':ent})
...
def emploiDuTemps(request):
	...
	debutMoisSuivantSuivant=datetime((debutMoisCourant+timedelta(64)).year,(debutMoisCourant+timedelta(64)).month,1)
	if(deb.weekday()>0):
	...
	for i in range(20):
		...
		preDefinis.append(("Semaine du "+str(dd.day)+"/"+str(dd.month)+" au "+str(ff.day)+"/"+str(ff.month),aAjouter))
